...
GRASP_DEPTH    = 0.05   # how far below object top the fingers should be when gripping
                        # e.g. 0.05 = fingers reach 5 cm into the object from top
...
# ─────────────────────────────────────────────────────────────────────────────
STABLE_SEC        = 3.0   # how long the detection must be consistent
...
def gripper_quat(angle_deg=0.0):
    """(qx, qy, qz, qw) for gripper pointing straight down, rotated angle_deg."""
...
# ─────────────────────────────────────────────────────────────────────────────
class PickNodeRS(Node):
    """
    Pick-and-place node for RealSense depth-based detection.

    Subscribes to:
        /red_object_pose  (PoseStamped, frame=base) — 3-D centroid of red object
        /red_box_angle    (Float64, degrees)         — orientation 0 / 90 deg
        /joint_states     (JointState)
        /gripper_control  published (Int32MultiArray) → updatedcode2.py

    Grasp heights (all in base/world frame, Z axis up):
        hover_flange_z  = obj_z + GRIPPER_LENGTH + HOVER_GAP
        plunge_flange_z = obj_z + GRIPPER_LENGTH - GRASP_DEPTH   ← FULLY DYNAMIC

    Dynamic plunge logic:
        No matter what height the object is at, plunge_z is always computed
        directly from the real obj_z reported by the RealSense depth camera.
        Placing object on a box, shelf, or table all work automatically.

    State machine: IDLE → HOVER → PLUNGE → LIFT → PLACE_PLUNGE → PLACE_LIFT → IDLE

    'Place' puts the object back at the SAME (x, y, z) it was picked from.
    """

    def __init__(self):
        super().__init__('pick_node_rs')
        ...
        # Current pick target (set when stability gate passes)
        self.goal_x     = None
        self.goal_y     = None
        self.goal_z     = None   # object centroid Z in base frame
        self.goal_angle = 0.0

        # Precomputed flange heights for this pick cycle
        self.hover_z  = None
        self.plunge_z = None

        # Stability gate
        self.last_x = None   # position of last completed pick
        self.last_y = None
        self.pending_x     = None
        self.pending_y     = None
        self.pending_z     = None
        ...
        # Latest detection (updated every frame, even while moving)
        self._latest_x     = None
        self._latest_y     = None
        self._latest_z     = None
        ...
        self.create_subscription(
            PoseStamped, '/red_object_pose', self._pose_cb, 10)
        self.create_subscription(
    ...
    def _pose_cb(self, msg):
        """
        Called at camera rate.  Updates latest detection and, when idle,
        runs the stability gate to decide when to commit a pick.
        """
        ...
        # ── Same spot as last pick — nothing new to do ────────────────────────
    ...
    def _commit_pick(self, x, y, z):
        ...
        self.goal_y     = y
        self.goal_z     = z
        self.goal_angle = self._latest_angle

        # ── DYNAMIC plunge computation ────────────────────────────────────────
        # hover_z  : flange height so fingertips are HOVER_GAP above object top
        # plunge_z : flange height so fingertips are GRASP_DEPTH below object top
        # Both are computed directly from the real obj_z — works at any height.
        ...
        self.get_logger().info(
            ...
            f"angle={self.goal_angle:.0f} deg")
    ...
    def _result_cb(self, future):
        if   self.motion_stage == 'HOVER':
            ...
            self.get_logger().info("Retracted. Cycle complete.")
            self.sequence_timer = self.create_timer(1.0, self._finish_cycle)

    # ── Steps ─────────────────────────────────────────────────────────────────
    def _go_hover(self):
        self.get_logger().info(
            f"Moving to hover  z={self.hover_z:.3f} m ...")
    ...
    def _go_place_plunge(self):
        self._clear_timer()
        self.get_logger().info(
            f"Descending to place z={self.plunge_z:.3f} m (tight)...")
    ...
    def _finish_cycle(self):
        ...
        self.get_logger().info(
            "Cycle done. Waiting for object to move to a new position...")
        self._unlock()
